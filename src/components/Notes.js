...
const Notes = () => {
    ...
    //ref.current.click is used to use the modal
    const updateNote = (currentNote) => {
        ref.current.click();
        setNote({ id: currentNote._id, etitle: currentNote.title, edescription: currentNote.description, etag: currentNote.tag })
    }
    // here by using refclose we are clicking the close button internally after we update the note
    const handleClick = (e) => {
    ...
    // the ref is used for the modal copy pasted from bootstrap
    //we adding modaL for edit notes
    return (
        <>
            <AddNote />
